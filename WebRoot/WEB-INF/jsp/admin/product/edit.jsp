	...
	<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/admin/adminProduct_update.action"
		...
		<table cellSpacing="1" cellPadding="5" width="100%" align="center"
			...
			<tr>
				...
				<td class="ta_01" bgColor="#ffffff">
					<select name="isHot">
							<option value="1" <c:if test="${findByPid.isHot==1}">selected</c:if>>是</option>
							<option value="0" <c:if test="${findByPid.isHot==0}">selected</c:if>>否</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td class="ta_01" bgColor="#ffffff" colspan="3">
					<select name="csid">
							<c:forEach items="${cslist}" var="cs">
								<option value="${cs.csid}" <c:if test="${cs.csid == findByPid.csid}">selected</c:if>>${cs.csname}</option>
							</c:forEach>
					</select>
				</td>
			</tr>
